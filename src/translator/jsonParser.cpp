...
inline std::shared_ptr<AuroraObject> getGeometry(Transform cameraTransform, Json::Value &root, RenderEnvironment *renderEnv){
    ...
    std::shared_ptr<Shape> shp = std::shared_ptr<Shape>(new ObjTriangleMesh(objStack, objInv, subdlevels, path));
    std::shared_ptr<AuroraObject> aurObj = std::shared_ptr<AuroraObject>(new AuroraObject(name, renderEnv, shp, material));
    ...
    for( Json::ValueIterator itr = root.begin() ; itr != root.end() ; itr++ ) {
        ...
        if (key == "objects") {
            ...
            for( Json::ValueIterator objItr = objRoot.begin() ; objItr != objRoot.end() ; objItr++ ) {
                ...
                if (objType == "geometry"){
                    std::shared_ptr<AuroraObject> geo = getGeometry(*cameraTransform, *objItr, renderEnv);
